...
ROWS = [
    ("host",),
    ("kv", "Now", "Designer & Engineer"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "Next.js, TypeScript, Tailwind"),
    ("kv", "Backend", "Convex, Sanity, Astro"),
    ("kv", "Cloud", "Cloudflare Workers, Vercel"),
    ("kv", "Analytics", "GA4, GTM, Meta Pixel"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">diae3@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">diae3</tspan><tspan fill="{MUTED}">@</tspan>'
